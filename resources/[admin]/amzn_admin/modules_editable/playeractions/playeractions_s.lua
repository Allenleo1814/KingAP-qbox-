local PlayerActions = {
    ...
    ["playeraction:AddCarToGarage"] = function(source, targetId)
        ...
        if FRAMEWORK == 'qb' or FRAMEWORK == 'qbx' then
            ...
            local citizenid = exports.qbx_core and exports.qbx_core:GetPlayer(targetId)?.PlayerData?.citizenid
            if FRAMEWORK == 'qb' then
                local QBCore = exports['qb-core']:GetCoreObject()
                local Player = QBCore.Functions.GetPlayer(targetId)
                citizenid = Player and Player.PlayerData and Player.PlayerData.citizenid or citizenid
            end
    ...
    ["playeraction:SendBack"] = function(source, targetId)
        TriggerClientEvent('amzn_admin:client:GoBack', targetId)
        Logger.logPlayerAction(source, targetId, "SendBack", "Sent player back to their last location")
        return true, "Player sent back to their last location"
    end,
    ...
    ["playeraction:Spectate"] = function(source, targetId)
        ...
        TriggerClientEvent('amzn_admin:client:closeAdminMenu', source)
        TriggerClientEvent('amzn_admin:client:spectate', source, targetId, targetCoords, targetName)
    ...
    ["playeraction:Ban"] = function(source, targetId, inputs)
        ...
        -- Prevent banning other admins (anyone in the permission cache)
        ...
        -- Insert into admin_punishment_list
        MySQL.insert('INSERT INTO admin_punishment_list (license, player_name, reason, issued_by, issued_at, status, type, expire) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
            targetLicense,
            targetName,
            reason,
    ...
    ["playeraction:Screenshot"] = function(source, targetId)
        ...
        exports.screencapture:serverCapture(targetId, {
            ...
            local imageSRC = "data:image/webp;base64," .. data
            p:resolve(imageSRC)
        end, "base64")
    ...
    ["playeraction:Warn"] = function(source, targetId, inputs)
        ...
        -- Send warning NUI message to the player
...
-- Register the callback for executing player actions
lib.callback.register('amzn_admin:executePlayerAction', function(source, data)
    ...
    -- If successful, notify the target player
    if success then
